...
if(parent==NULL)
{
  ...
  for(i = 0; i < count; i++)
    ...
    else if (parent==NULL && count<=1)
    {
        cout<<" skip internal print "<<endl;
    }
...
{  // to be written by students
    ...
    //cout<<" internal node count is "<<count<<endl;
    if( this->getCount()<=1 && this->parent==0x0 )
           
       {
        
         // leafing=new
           
         //  this->removesroot(value);
...
void InternalNode::removesroot(int value)
{
    count=0;
...
void InternalNode::updateKeysRoot()
{
    for(int i =0; i < this->getCount(); i++)
    {
        keys[i] = children[i]->values[i];
    }
