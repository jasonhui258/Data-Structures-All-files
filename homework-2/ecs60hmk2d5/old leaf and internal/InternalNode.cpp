...
{  // to be written by students
    ...
    for(int i=1; i<count; i++)
    {
        if(keys[i]>value)
        {
            ...
            //children[i-1]->count=count-1;
           
           /* if(children[i-1]->getCount()==0)
            {
               delete children[i-1];
                return NULL;
            }*/
            
           //  cout<<"keys is now "<<keys[i]<<endl;
            //keys[i]=NULL;
         break;
        }
...
void InternalNode::deleteChild(BTreeNode* dog)
{
    ...
    {
        keys[p]=children[p+1]->getMinimum();
    }
